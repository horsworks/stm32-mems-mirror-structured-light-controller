...
void Comm_OnRxISR(Commander_t *cmd) {
    ...
    // 简单命令解析
    if (strstr((char *)cmd->cmd_buffer, "start") != NULL) {
        cmd->stop_request = 0;
        // 清空 buffer 防止重复触发
        cmd->cmd_idx = 0;
        memset(cmd->cmd_buffer, 0, sizeof(cmd->cmd_buffer));
    ...
    if (strstr((char *)cmd->cmd_buffer, "stop") != NULL) {
        cmd->stop_request = 1;
        // 清空 buffer 防止重复触发
        cmd->cmd_idx = 0;
        memset(cmd->cmd_buffer, 0, sizeof(cmd->cmd_buffer));
    ...
    if (strstr((char *)cmd->cmd_buffer, "next") != NULL) {
        // 直接切换下一行，不需要主循环参与
    ...
    if (strstr((char *)cmd->cmd_buffer, "switch") != NULL) {
        // 直接切换下一行，不需要主循环参与
...
void Comm_HandleUpdateProcess(Commander_t *cmd, Projector_t *p) {
    ...
    if (new_rows > MAX_ROWS || new_cols > MAX_COLS || new_rows == 0) {
